...
function ReportCardLike({ reporte }: { reporte: ReporteZonaOscura }) {
  return (
    <div className="theme-surface rounded-2xl border border-outline-variant bg-white p-md max-lg:border-slate-700 max-lg:bg-white/[0.06]">
      <div className="flex items-start justify-between gap-md">
        <div className="min-w-0">
          <p className="font-mono text-[12px] font-semibold text-slate-500 max-lg:text-slate-400">{reporte.codigo}</p>
          <h3 className="theme-text-primary mt-xs font-subtitulo text-[17px] font-semibold text-primary max-lg:text-white">{reporte.direccion}</h3>
          <p className="theme-text-muted mt-xs text-sm text-on-surface-variant max-lg:text-slate-400">{reporte.distrito}</p>
        </div>
        <ReportStatusBadge estado={reporte.estado} />
      </div>
      <div className="mt-md flex items-center justify-between gap-md border-t border-outline-variant pt-sm max-lg:border-slate-700">
        <span className="theme-text-muted text-sm text-on-surface-variant max-lg:text-slate-300">{etiquetasTipoProblema[reporte.tipoProblema]}</span>
